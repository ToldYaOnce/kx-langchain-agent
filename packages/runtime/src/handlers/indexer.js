    ...
    try {
        ...
        for (const record of event.Records) {
            ...
            try {
                ...
                // Emit success event
                await eventBridgeService.publishAgentTrace(EventBridgeService.createAgentTraceEvent(tenantId, 'rag.index.updated', {
                    metadata: {
                        bucket: bucketName,
                        key: objectKey,
                ...
                // Emit error event
                await eventBridgeService.publishAgentError(EventBridgeService.createAgentErrorEvent(tenantId, `Failed to index document: ${docError instanceof Error ? docError.message : 'Unknown error'}`, {
                    context: {
                        bucket: bucketName,
                        key: objectKey,
                        event_name: record.eventName,
