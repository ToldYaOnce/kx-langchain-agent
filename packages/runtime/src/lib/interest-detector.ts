...
export class InterestDetector {
  ...
  analyzeMessage(message: string, conversationHistory?: string[]): InterestAnalysis {
    ...
    let urgencyLevel: 'urgent' | 'normal' | 'casual';
   ...
   */
  private analyzeConversationHistory(history: string[]): InterestAnalysis {
    let totalPositive = 0;
    let totalNegative = 0;
    let totalUrgency = 0;
    let totalCasual = 0;
  ...
  detectObjections(message: string): {
    ...
    const messageLower = message.toLowerCase();
    const foundObjections: string[] = [];
    let primaryType: 'price' | 'time' | 'commitment' | 'location' | 'general' | 'none' = 'none';
