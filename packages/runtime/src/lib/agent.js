...
import { ResponseChunker } from './response-chunker.js';
...
export class AgentService {
    ...
    async processMessage(context, existingHistory) {
        ...
        try {
            ...
            if (this.config.eventBridgeService) {
                ...
                await this.config.eventBridgeService.publishAgentTrace(EventBridgeService.createAgentTraceEvent(context.tenantId, 'agent.message.processed', {
            ...
            if (this.config.eventBridgeService) {
                await this.config.eventBridgeService.publishAgentError(EventBridgeService.createAgentErrorEvent(context.tenantId, error instanceof Error ? error.message : 'Unknown error', {
                    contactPk: DynamoDBService.createContactPK(context.tenantId, context.email_lc),
    ...
    async processMessageChunked(context) {
        ...
        return ResponseChunker.chunkResponse(fullResponse, context.source, currentPersona.responseChunking);
    }
    /**
