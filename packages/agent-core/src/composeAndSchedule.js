...
import { PERSONAS } from "./personaTimings";
const sqs = new SQSClient({});
function prng(seed) {
    return () => (seed = Math.imul(48271, seed) % 2147483647) / 2147483647;
}
function sample(r, [lo, hi]) {
    return Math.round(lo + r() * (hi - lo));
}
export function computeTiming(seedKey, persona, inputChars, inputTokens, replyChars) {
...
export async function scheduleActions(params) {
    const { queueUrl, tenantId, contact_pk, conversation_id, channel, personaName, message_id, replyText, inputChars, inputTokens } = params;
...
//# sourceMappingURL=composeAndSchedule.js.map