...
export async function handler(event, context) {
    ...
    try {
        ...
        await eventBridgeService.publishAgentReply(EventBridgeService.createAgentReplyEvent(event.tenantId, DynamoDBService.createContactPK(event.tenantId, event.email_lc), response, preferredChannel, routing, {
        ...
        try {
            const config = loadRuntimeConfig();
            const eventBridgeService = new EventBridgeService(config);
            await eventBridgeService.publishAgentError(EventBridgeService.createAgentErrorEvent(event.tenantId, error instanceof Error ? error.message : 'Unknown agent error', {
 ...
 */
export async function structuredHandler(event, context) {
